...
export async function syncUsersToGithub(
  ...
  const path = cfg.dataPath || 'src/data.json';
  const existing = await getGithubFile(cfg, path);

  // Preserve any __config__ block already in the file so site settings survive
  ...
  const payload = Object.keys(existingConfig).length > 0
    ? { users, __config__: existingConfig }
    : users;
...
export async function pullFromGithub(
  ...
  if (!excludeUsers) {
    try {
      ...
      if (dataFile) {
        const raw = JSON.parse(dataFile.content);
        // Support plain array OR {users:[], __config__:{}} format
        if (Array.isArray(raw)) {
          result.users = raw as UserRecord[];
        } else if (Array.isArray(raw?.users)) {
          result.users = raw.users as UserRecord[];
          // Merge __config__ into appConfig if present and appConfig not yet loaded
          if (raw.__config__ && !result.appConfig) {
            result.appConfig = raw.__config__ as Partial<AppConfig>;
          }
        }
      }
    } catch (e) {
