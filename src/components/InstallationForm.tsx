cat > /home/claude/InstallationForm.tsx << 'ENDOFFILE'
/**
 * InstallationForm.tsx — نموذج التركيبات الديناميكي الكامل
 *
 * ✅ Controlled State مؤمّنة بالكامل — لا useRef لا defaultValue
 * ✅ countInput (string) منفصلة عن clients[] — لا crash أثناء الكتابة
 * ✅ clients[] تُعدَّل فقط عند رقم صحيح >= 1
 * ✅ onBlur يعيد إلى "1" عند قيمة فارغة أو خاطئة
 * ✅ أزرار + و - يحدّثان countInput و clients معاً بشكل متزامن
 * ✅ safeClients + Optional Chaining في كل JSX
 * ✅ بدون <form> — onClick فقط
 */
...
// ─── Helper: توليد مصفوفة عملاء بأمان ────────────────────────────────────────

function buildClientsList(targetCount: number, current: ClientEntry[]): ClientEntry[] {
  const safe    = isNaN(targetCount) || targetCount < 1 ? 1 : Math.floor(targetCount);
  const base    = Array.isArray(current) && current.length > 0 ? current : [emptyClient()];
  if (safe === base.length) return base;
  if (safe > base.length) {
    return [
      ...base,
      ...Array.from({ length: safe - base.length }, () => emptyClient()),
    ];
  }
  return base.slice(0, safe);
}
...
export default function InstallationForm({
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // ✅ الحل الجذري:
  //    countInput  → State نصية خالصة تعكس ما يكتبه المستخدم (قد تكون "")
  //    clients[]   → تُعدَّل فقط عند رقم صحيح >= 1 من onChange أو onBlur أو +/-
  // ──────────────────────────────────────────────────────────────────────────
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // onChange: يحدّث countInput دائماً (حرية الكتابة)
  //           يحدّث clients[] فقط عند رقم صحيح >= 1
  // ──────────────────────────────────────────────────────────────────────────
  const handleCountChange = useCallback((raw: string) => {
    setCountInput(raw);
    const parsed = parseInt(raw, 10);
    if (!isNaN(parsed) && parsed >= 1) {
      setClients((prev) => buildClientsList(parsed, prev));
    }
    // إذا raw فارغة أو NaN → لا تلمس clients[] — فقط انتظر onBlur
  }, []);

  // ──────────────────────────────────────────────────────────────────────────
  // onBlur: حماية نهائية — إذا الحقل فارغ أو خاطئ → أرجع إلى 1
  // ──────────────────────────────────────────────────────────────────────────
  const handleCountBlur = useCallback(() => {
    const parsed = parseInt(countInput, 10);
    if (isNaN(parsed) || parsed < 1) {
      setCountInput('1');
      setClients((prev) => buildClientsList(1, prev));
    } else {
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // أزرار + و - : يحدّثان countInput و clients معاً في نفس الدالة بشكل متزامن
  // ──────────────────────────────────────────────────────────────────────────
  const handleIncrement = useCallback(() => {
    const current = parseInt(countInput, 10);
    const next    = isNaN(current) || current < 1 ? 2 : current + 1;
    setCountInput(String(next));
    setClients((prev) => buildClientsList(next, prev));
  }, [countInput]);
  ...
  // ── العدد الفعلي من المصفوفة ───────────────────────────────────────────────
  const actualCount =
    Array.isArray(clients) && clients.length > 0 ? clients.length : 1;

  // ── Validation ─────────────────────────────────────────────────────────────
  const validate = (): string[] => {
    const errs: string[] = [];
    if (!workerName.trim()) errs.push('يرجى كتابة اسم العامل');
    const safe = Array.isArray(clients) && clients.length > 0
      ? clients : [emptyClient()];
    safe.forEach((c, i) => {
  ...
  const handleSubmitClick = async () => {
    ...
    try {
      ...
      if (extra.length > 0)
        customFields['__additionalClients'] = JSON.stringify(extra);
  ...
  // ── Styles ─────────────────────────────────────────────────────────────────
  const inputCls =
    'w-full px-4 py-3 rounded-xl border border-slate-200 text-sm outline-none' +
    ' focus:ring-2 focus:ring-amber-300 bg-white text-slate-700 transition';
  const labelCls = 'block text-xs font-bold text-slate-600 mb-1.5';

  // ✅ safeClients: مصدر الـ JSX الوحيد — مضمونة دائماً
          ...
          <div>
            ...
            {workers.length > 0 && (
              <p className="text-[10px] text-slate-400 mt-1">
                ...
                لاختيار عامل سابق، أو اكتب اسماً جديداً مباشرة
              </p>
            )}
          </div>

          {/* ══ عدد التركيبات ══
               ✅ countInput: Controlled string — حرية كتابة كاملة
               ✅ onChange: يحدّث countInput دائماً، clients[] فقط عند رقم صحيح >= 1
               ✅ onBlur:   حماية نهائية — يعيد إلى "1" عند قيمة خاطئة
               ✅ +/-:      يحدّثان الاثنين معاً بشكل متزامن في نفس الدالة         */}
          <div>
            <label className={labelCls}>
            ...
            <div className="flex items-center gap-3">
              ...
              {/* حقل الإدخال — Controlled بـ countInput */}
              <input
                type="text"
                inputMode="numeric"
                value={countInput}
                ...
                <div className="mt-2 pt-3 border-t border-amber-200">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    ...
                    <PhotoSlot
                      ...
                      accept="image/*"
                      onFileChange={(f) =>
                        handleFileUpload(f, index, 'clientIdPhoto', `clientId-${index}`)
                      }
                    ...
                    <PhotoSlot
                      ...
                      accept="image/*"
                      onFileChange={(f) =>
                        handleFileUpload(f, index, 'boxPhoto', `box-${index}`)
                      }
                    ...
                    <PhotoSlot
                      ...
                      accept="image/*"
                      onFileChange={(f) =>
                        handleFileUpload(f, index, 'thermalPhoto', `thermal-${index}`)
                      }
                    ...
                    <PhotoSlot
                      ...
                      accept="image/*"
                      onFileChange={(f) =>
                        handleFileUpload(f, index, 'mainBoxPhoto', `mainBox-${index}`)
                      }
                    ...
                    <PhotoSlot
                      ...
                      accept="video/*"
                      onFileChange={(f) =>
                        handleFileUpload(f, index, 'installationVideo', `video-${index}`)
                      }
