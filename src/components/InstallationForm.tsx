 ...
 *
 * ✅ إصلاح White Screen نهائياً:
 *    - حقل "عدد التركيبات" Uncontrolled (ref + defaultValue) — لا onChange لا re-render
 *    - clients[] تُعدَّل فقط في onBlur وأزرار + و - بعد التحقق من القيمة
 *    - safeClients في الـ JSX + Optional Chaining في كل وصول
 * ✅ اسم العامل: حقل نص حر + قائمة اختيارية — Validation يقرأ workerName مباشرة
 * ✅ Dynamic Loop: قسم كامل (حقول + مرفقات) لكل عميل
...
export default function InstallationForm({
  ...
  // ── قائمة العملاء — تُعدَّل فقط من applyCount ──────────────────────────────
  ...
  // ────────────────────────────────────────────────────────────────────────────
  // ✅ countRef: مرجع مباشر لحقل الـ input — Uncontrolled تماماً
  //    لا onChange، لا re-render، لا crash أثناء الكتابة
  // ────────────────────────────────────────────────────────────────────────────
  const countInputRef = useRef<HTMLInputElement>(null);

  // ── applyCount: الدالة الوحيدة التي تُعدِّل clients[] ──────────────────────
  const applyCount = useCallback((rawValue: number | string) => {
    // ① تحويل القيمة لرقم صحيح
    const parsed = typeof rawValue === 'string'
      ? parseInt(rawValue.trim(), 10)
      : Math.floor(rawValue);

    // ② تأمين: NaN أو أقل من 1 → 1
    const safe = isNaN(parsed) || parsed < 1 ? 1 : parsed;

    // ③ مزامنة الـ input المرئي مع القيمة الآمنة
    if (countInputRef.current) {
      countInputRef.current.value = String(safe);
    }

    // ④ تحديث المصفوفة دفعة واحدة بـ Array.from
    setClients((prev) => {
  ...
  // ── onBlur للـ input: يُفعَّل فقط عند مغادرة الحقل ────────────────────────
  const handleCountBlur = useCallback(() => {
    const val = countInputRef.current?.value ?? '1';
    applyCount(val);
  }, [applyCount]);

  // ── أزرار + و - ────────────────────────────────────────────────────────────
  const handleIncrement = useCallback(() => {
    const current = parseInt(countInputRef.current?.value ?? '1', 10);
  ...
  const resetForm = () => {
    setWorkerName('');
    if (countInputRef.current) countInputRef.current.value = '1';
  ...
  // ✅ safeClients: مضمونة دائماً في الـ JSX — تمنع كل crash محتمل
    ...
    <div className="w-full max-w-2xl mx-auto" dir="rtl">
      <div className="bg-white rounded-3xl shadow-lg border border-slate-100 overflow-hidden">
        ...
        <div className="p-6 space-y-5">
          ...
          {/* ══ عدد التركيبات — Uncontrolled input ══
               ✅ لا onChange — لا re-render أثناء الكتابة
               ✅ onBlur فقط يُفعِّل applyCount بعد انتهاء الكتابة          */}
          <div>
            <label className={labelCls}>
            ...
            <div className="flex items-center gap-3">
              ...
              {/*
                ✅ Uncontrolled: defaultValue بدلاً من value
                ✅ لا onChange — لا re-render لحظي
                ✅ onBlur فقط يحدث المصفوفة بعد اكتمال الكتابة
              */}
              <input
                ref={countInputRef}
                type="text"
